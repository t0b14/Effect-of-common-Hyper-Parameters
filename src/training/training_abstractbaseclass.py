...
class ABCTrainingModule(ABC):
    ...
    def fit(self, num_epochs: int = 100):
        ...
        for cur_epoch in (pbar_epoch := tqdm(range(num_epochs))):
            ...
            self.model.eval()
            self.val_loss = 0
            self.train(gradients, seq_l, self.n_intervals, self.device, self.val_dataloader, self.step)
    ...
    def step(self, inputs, labels, h_1=None, gradients=None, eval=False, backprop=True, trial_loss=None):
        """Returns loss"""
    ...
    def create_hidden_state_plot(self, cur_epoch, num_epochs):
        if(1. * (cur_epoch) / num_epochs >= 1. / (self.n_weight_histograms-1) * self.cur_hidden_state_hist):
            ...
            self.save_histogram(start_h_1_container.view(-1).detach().numpy(), "t_start_hidden_state", cur_epoch + 1, num_epochs)
            self.save_histogram(mid_h_1_container.view(-1).detach().numpy(), "t_mid_hidden_state", cur_epoch + 1, num_epochs)
            self.save_histogram(end_h_1_container.view(-1).detach().numpy(), "t_end_hidden_state", cur_epoch + 1, num_epochs)
